...
var getTrackStyle = function (el) {  
    ...
    currentcat=curLabel[0].textContent;
    if(current_id!=-1){
        catday(currentcat,currenthour,current_id);
    }
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            result.forEach(function(element){   
                ...
                feature['properties'] =  {'name': element.name,'id':element.id,'total_pop': element.total_pop};
                L.geoJSON(feature, {
                    style: {
                        weight: 2,
                        opacity: 0.5,
                    ...
                    mymap.setView(this.getBounds().getCenter(),14.4);
                    catday($('.range-labels li.active.selected')[0].innerHTML,$('.range-hour li.active.selected')[0].innerHTML,feature.properties.id);
...
function catday(cat,hour,id){
    ...
    xhttp.open("GET", "http://localhost:8080/RoadSection/Town/"+id+"/"+cat+"/"+hour, true);
    xhttp.send();
}
towns();